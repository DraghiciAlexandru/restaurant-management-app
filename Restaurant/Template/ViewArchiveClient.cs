...
namespace Restaurant.Template
{
    class ViewArchiveClient : Panel
    {
        ...
        private void Pic_Click(object sender, EventArgs e)
        {
            ...
            controlOrders = new ControlOrders();
            ListaSimpla<Order> list = controlOrders.getClient(ControlCustomers.loged.ID);
        ...
        {
            for (int i = 0; i < controlOrders.getClient(ControlCustomers.loged.ID).size(); i++)
            {
                CardArchiveStaff cardArchive = new CardArchiveStaff(controlOrders.getClient(ControlCustomers.loged.ID).getAtPosition(i));

                cardArchive.dateTime.Value = controlBookings.getByOrder(controlOrders.getClient(ControlCustomers.loged.ID).getAtPosition(i).Id).Date;
