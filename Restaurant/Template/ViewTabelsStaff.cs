...
namespace Restaurant.Template
{
    class ViewTabelsStaff : Panel
    {
        ...
        private void setControl()
        {
            for (int i = 0; i < tabels.size(); i++)
            {
                ...
                if ((controlBookings.isRezerved(booking) == true) && (controlCancel.isCancel(booking) == false))
                {
                    ...
                    tabels.getAtPosition(i).Click += ViewTabelsStaff_Click;
                }
                else
                {
        ...
        private void ViewTabelsStaff_Click(object sender, EventArgs e)
        {
            Button btn = sender as Button;

            CardTabelStaff card = new CardTabelStaff(controlBookings.getByDate(new Booking(0, 0, 0, int.Parse(btn.Text), DateTime.Now)));
        ...
        private void setNotViz()
        {
            for(int i=0; i<tabels.size(); i++)
            {
                tabels.getAtPosition(i).Visible = false;
            }
        }
    }
}
